...
根据题目描述，我们应该尽可能多地取标记为1的物品，然后取标记为0的物品，最后取标记为-1的物品。

因此：

-   如果袋子中的物品标记为1的数量大于等于k，那么取k件物品，数字之和为k；
-   如果袋子中的物品标记为1的数量小于k，那么取numOnes件物品，数字之和为numOnes；如果标记为0的物品数量大于等于k - numOnes，那么再取k - numOnes件物品，数字之和还是numOnes；
-   否则，我们再从标记为-1的物品中取k - numOnes - numZeros件物品，数字之和为numOnes - (k - numOnes - numZeros)。
