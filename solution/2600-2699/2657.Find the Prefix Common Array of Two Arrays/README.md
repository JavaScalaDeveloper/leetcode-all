...
我们可以使用两个数组cnt1和cnt2分别记录数组A和B中每个元素出现的次数，用数组ans记录答案。

遍历数组A和B，将A[i]在cnt1中出现次数加一，将B[i]在cnt2中出现次数加一。然后枚举j \in [1,n]，计算cnt1和cnt2中每个元素j出现次数的最小值，累加到ans[i]中。

遍历结束后，返回答案数组ans即可。

时间复杂度O(n^2)，空间复杂度O(n)。其中n是数组A和B的长度。
