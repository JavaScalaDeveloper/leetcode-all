...
接下来，我们从(r, c)开始进行深度优先搜索，每次搜索位置(i, j)时，我们先判断g[i][j]是否等于m × n - 1，若是，说明我们已经找到了一种方案，此时将ok置为 `true` 并且返回。否则我们枚举骑士的八个移动方向对应的位置(x, y)，若满足0 ≤ x < m,0 ≤ y < n，并且g[x][y]=-1，那么我们将g[x][y]更新为g[i][j]+1，然后递归搜索位置(x, y)。如果搜索结束后，变量ok为 `true`，则直接返回。否则，我们将g[x][y]重置为-1，继续搜索其他方向。

最后返回二维数组g即可。

时间复杂度O(8^{m × n})，空间复杂度O(m × n)。其中m和n为题目给定的整数。
