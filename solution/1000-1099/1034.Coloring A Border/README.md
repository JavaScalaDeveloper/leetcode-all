...
我们从位置(row, col)出发，利用 DFS 搜索所有颜色为grid[row][col]的网格块，如果该网格块的某个相邻位置的颜色不为grid[row][col]，或者该网格块在网格的边界上，则将该网格块的颜色改为color。

时间复杂度O(m \times n)，空间复杂度O(m \times n)。其中m和n分别是网格的行数和列数。
