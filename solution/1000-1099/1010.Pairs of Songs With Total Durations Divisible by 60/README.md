...
如果一个数对(a, b)之和能被60整除，即(a + b) \bmod 60 = 0，那么(a \bmod 60 + b \bmod 60) \bmod 60 = 0，不妨记x=a \bmod 60,y = b \bmod 60，那么有(x + y) \bmod 60 = 0，即y=(60 - x) \bmod 60。

因此，我们可以遍历歌曲列表，用一个长度为60的数组cnt记录每个余数x出现的次数。对于当前的x，如果数组cnt中存在余数y = (60 - x) \bmod 60，那么将cnt[y]累加进答案中。然后，将x在数组cnt中的出现次数加1。继续遍历，直到遍历完整个歌曲列表。

遍历结束后，即可得到满足条件的歌曲对数目。

时间复杂度O(n)，空间复杂度O(C)。其中n是歌曲列表的长度；而C是余数的可能取值，这里C=60。
