...
我们先将a, b, c排序，记为x, y, z，即x \lt y \lt z。

接下来分情况讨论：

1. 如果z - x \leq 2，说明3个数已经相邻，不用移动，结果为[0, 0]；
1. 否则，如果y - x \lt 3，或者z - y \lt 3，说明有两个数只间隔一个位置，我们只需要把另一个数移动到这两个数的中间，最小移动次数为1；其他情况，最小移动次数为2；
