...
定义dp[i][j]表示字符串s中下标范围[i..j]内的子串是否为有效括号字符串。答案为dp[0][n - 1]。

子串长度为1时，如果字符s[i]为 `*`，则dp[i][i]为 `true`，否则为 `false`。

子串长度大于1时，如果满足下面任意一种情况，则dp[i][j]为 `true`：

-   子串s[i..j]的左边界为 `(` 或 `*`，且右边界为 `*` 或 `)`，且s[i+1..j-1]为有效括号字符串；
-   子串s[i..j]中的任意下标k，如果s[i..k]为有效括号字符串，且s[k+1..j]为有效括号字符串，则s[i..j]为有效括号字符串。

时间复杂度O(n^3)，空间复杂度O(n^2)。其中n为字符串 `s` 的长度。

**方法二：贪心 + 两遍扫描**

两遍扫描，第一遍从左往右，确定每一个右括号都可以成功配对，第二遍从右往左，确定每一个左括号都可以成功配对。

时间复杂度O(n)，空间复杂度O(1)。其中n为字符串 `s` 的长度。
