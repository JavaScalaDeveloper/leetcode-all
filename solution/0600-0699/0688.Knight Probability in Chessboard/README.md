...
<p><img src="https://gcore.jsdelivr.net/gh/doocs/leetcode@main/solution/0600-0699/0688.Knight%20Probability%20in%20Chessboard/images/knight.png" style="height: 300px; width: 300px;" /></p>
...
我们定义f[h][i][j]表示骑士从(i, j)位置出发，走了h步以后还留在棋盘上的概率。那么最终答案就是f[k][row][column]。

当h=0时，骑士一定在棋盘上，概率为1，即f[0][i][j]=1。

当h > 0时，骑士在(i, j)位置上的概率可以由其上一步的8个位置上的概率转移得到，即：


f[h][i][j] = \sum_{a, b} f[h - 1][a][b] × \frac{1}{8}


其中(a, b)是从(i, j)位置可以走到的8个位置中的一个。

最终答案即为f[k][row][column]。

时间复杂度O(k × n^2)，空间复杂度O(k × n^2)。其中k和n分别是给定的步数和棋盘大小。
