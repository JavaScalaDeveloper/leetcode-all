...
将arr中的所有元素按照与x的距离从小到大进行排列。取前k个元素排序后返回。

时间复杂度O(nlogn)。

**方法二：双指针**

直觉上，有序数组arr最靠近x的k个数必然是一段连续的子数组。

我们可以声明头尾指针，记为l和r，然后根据x-arr[l]与arr[r-1] - x的大小比较结果缩小范围，直到r - l = k。

时间复杂度O(n)。

**方法三：二分查找**

在方法二的基础上，我们更进一步，查找大小为k的窗口的左边界。
