...
定义两个指针i和j，分别指向字符串s和t的首字符。遍历字符串t，当s[i] \neq t[j]时，指针i后移，直到s[i] = t[j]或者i到达字符串s的末尾。如果i到达字符串s的末尾，说明t中的字符t[j]无法在s中找到对应的字符，返回t中剩余的字符数。否则，将指针i和j同时后移，继续遍历字符串t。

时间复杂度(m + n)，空间复杂度O(1)。其中m和n分别是字符串s和t的长度。
