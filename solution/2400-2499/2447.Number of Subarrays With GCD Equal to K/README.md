...
我们可以枚举nums[i]作为子数组的左端点，然后枚举nums[j]作为子数组的右端点，其中i\le j。在枚举右端点的过程中，我们可以用一个变量g来维护当前子数组的最大公因数，每次枚举到一个新的右端点时，我们更新最大公因数g = \gcd(g, nums[j])。如果g=k，那么当前子数组的最大公因数等于k，我们就将答案增加1。

枚举结束后，返回答案即可。

时间复杂度O(n \times (n + \log M))，其中n和M分别是数组 `nums` 的长度和数组 `nums` 中的最大值。
