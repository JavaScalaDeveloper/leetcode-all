...
然后遍历 `nums`，枚举中间元素nums[j]，在nums[j]左侧找到最近的下标i，使得nums[i] < nums[j]成立；在nums[j]右侧找到最近的下标k，使得nums[k] > nums[j]成立。那么以nums[j]作为中间元素，且符合条件的三元组数量为(i+1) × (n - k)，累加到答案中。
...
然后遍历哈希表cnt，枚举中间元素的个数b，左侧元素个数记为a，那么右侧元素个数有n-a-b，此时符合条件的三元组数量为a × b × c，累加到答案中。接着更新a=a+b，继续枚举中间元素的个数b。
