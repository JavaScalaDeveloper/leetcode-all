...
假设连通块的个数为k，那么要删除的边数为k-1，每个连通块的价值为\frac{s}{k}，其中s为 `nums` 所有节点的值之和。

我们从大到小枚举k，如果存在一个k，使得\frac{s}{k}是整数，并且得到的每个连通块的价值都相等，那么直接返回k-1。其中k的初始值为\min(n, \frac{s}{mx})，记mx为 `nums` 中的最大值。

关键点在于判断对于给定的\frac{s}{k}，是否能划分出若干子树，使得每棵子树的价值都为\frac{s}{k}。

这里我们通过 `dfs` 函数来判断，从上到下递归遍历求出各个子树的价值，如果子树价值和恰好为\frac{s}{k}，说明此时划分成功，我们将价值置为0返回给上一层，表示此子树可以与父节点断开。如果子树价值之和大于\frac{s}{k}，说明此时划分失败，我们返回-1，表示无法划分。

时间复杂度O(n\times \sqrt{s})，其中n和s分别为 `nums` 的长度和 `nums` 所有节点的值之和。
