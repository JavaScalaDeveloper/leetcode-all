...
我们用变量last记录上一个任务的结束时间，用变量mx记录最长的工作时间，用变量ans记录工作时间最长且id最小的员工。初始时，三个变量均为0。

接下来，遍历数组logs，对于每个员工，我们将员工完成任务的时间减去上一个任务的结束时间，即可得到该员工的工作时间t。如果mx小于t，或者mx等于t且该员工的id小于ans，则更新mx和ans。然后我们将last更新为上一个任务的结束时间加上t。继续遍历，直到遍历完整个数组。

最后返回答案ans即可。

时间复杂度O(n)，空间复杂度O(1)。其中n为数组logs的长度。
