...
遍历 `queries` 中的每个单词，对于每个单词，遍历 `dictionary` 中的每个单词，判断两个单词不同字符的位置数是否小于3，如果是，则将该单词加入结果集。

时间复杂度O(m\times n\times k)。其中m和n分别是 `queries` 和 `dictionary` 的长度，而k是 `queries` 和 `dictionary` 中单词的长度。
