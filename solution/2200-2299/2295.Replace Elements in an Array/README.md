...
我们先用哈希表d记录数组 `nums` 中每个数字的下标，然后遍历操作数组 `operations`，对于每个操作[a, b]，我们将a在 `nums` 中的下标d[a]对应的数字替换为b，并更新d中b的下标为d[a]。

最后返回 `nums` 即可。

时间复杂度O(n + m)，空间复杂度O(n)。其中n和m分别是数组 `nums` 和 `operations` 的长度。
