...
我们可以枚举子数组的左右端点i和j，其中0 \leq i \leq j < n。对于每个子数组nums[i,..j]，我们可以统计其中可以被p整除的元素的个数cnt，如果cnt \leq k，则该子数组满足条件。我们将所有满足条件的子数组的元素序列作为字符串存入哈希表中，最后哈希表中的元素个数即为答案。

时间复杂度O(n^3)，空间复杂度O(n^2)。其中n为数组nums的长度。
