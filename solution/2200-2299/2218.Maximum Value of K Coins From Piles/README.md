...
对每个栈求前缀和s，s_i视为一个体积为i且价值为s_i的物品。

问题转化为求从n个物品组中取物品体积为k，且每组最多取一个物品时的最大价值和。

定义dp[i][j]表示从前i个组中取体积之和为j的物品时的最大价值和。

枚举第i组所有物品，设当前物品体积为w，价值为v，则有f[i][j]=max(f[i][j],f[i-1][j-w]+v)。
