...
我们枚举中位数的下标i，其中x-1≤ i≤ len(arr)-y，那么左侧数组的前缀和ls=s[i+1]-s[i+1-x]，右侧数组的前缀和rs=s[i+1+y]-s[i+1]。当前中位数在 `nums` 中的下标为j=arr[i]，将左侧x个元素移动到[j-x+1,..j]所需要的操作次数为a=(j+j-x+1)×\frac{x}{2}-ls，将右侧y个元素移动到[j+1,..j+y]所需要的操作次数为b=rs-(j+1+j+y)×\frac{y}{2}，那么总的操作次数为a+b，我们取所有总的操作次数的最小值即可。
