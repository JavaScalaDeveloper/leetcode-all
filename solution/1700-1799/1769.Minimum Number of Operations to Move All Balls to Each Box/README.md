...
我们可以预处理出每个位置i左边的小球移动到i的操作数，记为left[i]；每个位置i右边的小球移动到i的操作数，记为right[i]。那么答案数组的第i个元素就是left[i] + right[i]。

时间复杂度O(n)，空间复杂度O(n)。其中n为 `boxes` 的长度。

我们还可以进一步优化空间复杂度，只用一个答案数组ans以及若干个变量即可。

时间复杂度O(n)，忽略答案数组的空间消耗，空间复杂度O(1)。其中n为 `boxes` 的长度。
