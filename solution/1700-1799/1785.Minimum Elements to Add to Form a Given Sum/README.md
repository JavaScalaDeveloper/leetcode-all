...
我们先计算数组元素总和s，然后计算s与goal的差值d。

那么需要添加的元素数量为d的绝对值除以limit向上取整，即\lceil \frac{|d|}{limit} \rceil。

注意，本题中数组元素的数据范围为[-10^6, 10^6]，元素个数最大为10^5，总和s以及差值d可能会超过32位整数的表示范围，因此需要使用64位整数。
