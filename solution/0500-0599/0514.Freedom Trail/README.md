...
然后我们定义f[i][j]表示拼写完字符串key的前i+1个字符，且ring的第j个字符与12:00方向对齐的最少步数。初始时f[i][j]=+\infty。答案为\min_{0 \leq j < n} f[m - 1][j]。
...
接下来，我们考虑当i \geq 1时，状态如何转移。我们可以枚举key[i]在ring中的位置列表pos[key[i]]，并枚举key[i-1]在ring中的位置列表pos[key[i-1]]，然后更新f[i][j]，即f[i][j]=\min_{k \in pos[key[i-1]]} f[i-1][k] + \min(\text{abs}(j - k), n - \text{abs}(j - k)) + 1。

最后，我们返回\min_{0 \leq j \lt n} f[m - 1][j]即可。

时间复杂度O(m \times n^2)，空间复杂度O(m \times n)。其中m和n分别是字符串key和ring的长度。
