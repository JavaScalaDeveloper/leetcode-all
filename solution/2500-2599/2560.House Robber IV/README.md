...
题目求的是窃贼的最小窃取能力，我们可以二分枚举窃贼的窃取能力，对于枚举的能力x，我们可以通过贪心的方式判断窃贼是否能够窃取至少k间房屋，具体地，我们从左到右遍历数组，对于当前遍历到的房屋i，如果nums[i] \leq x且i与上一个窃取的房屋的下标之差大于1，则窃贼可以窃取房屋i，否则窃贼不能窃取房屋i。累计窃取的房屋数，如果窃取的房屋数大于等于k，则说明窃贼可以窃取至少k间房屋，此时窃贼的窃取能力x可能是最小的，否则窃贼的窃取能力x不是最小的。

时间复杂度O(n \times \log m)，空间复杂度O(1)。其中n和m分别是数组 `nums` 的长度和数组 `nums` 中的最大值。
