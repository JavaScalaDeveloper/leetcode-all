...
我们观察发现，如果在格子(0, 0)处无法移动，即grid[0][1] > 1且grid[1][0] > 1，那么我们在格子(0, 0)无法再移动，此时返回-1即可。而对于其他情况，我们都可以移动。
...
我们每次从优先队列中取出当前最早到达的格子(t, i, j)，如果(i, j)为(m - 1, n - 1)，那么我们直接返回t即可，否则，我们遍历(i, j)的上下左右四个相邻格子(x, y)，如果t + 1 < grid[x][y]，那么我们移动到(x, y)的时间nt = grid[x][y] + (grid[x][y] - (t + 1)) mod 2，此时我们可以通过反复移动将时间拉长至不小于grid[x][y]，这取决于t + 1和grid[x][y]距离的奇偶性。否则，我们移动到(x, y)的时间nt = t + 1。如果nt < dist[x][y]，那么我们更新dist[x][y] = nt，并将(nt, x, y)加入优先队列中。
