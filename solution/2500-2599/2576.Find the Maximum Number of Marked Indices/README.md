...
为了将下标尽可能多地标记，我们可以将数组 `nums` 排序，然后从左到右遍历数组，对于每个下标i，我们在数组的右半部分找到第一个满足2 \times nums[i] \leq nums[j]的下标j，然后标记下标i和j。继续遍历下一个下标i。当我们遍历完数组的右半部分时，说明标记已经完成，此时标记的下标数目即为答案。
