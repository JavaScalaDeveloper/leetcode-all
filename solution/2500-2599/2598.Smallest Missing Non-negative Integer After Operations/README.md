...
我们用哈希表或数组cnt统计数组中每个数对value取模后的余数的个数。

然后从0开始遍历，对于当前遍历到的数i，如果cnt[i mod value]为0，说明数组中不存在一个数对value取模后的余数为i，那么i就是数组的 MEX，直接返回即可。否则，将cnt[i mod value]减1，继续遍历。

时间复杂度O(n)，空间复杂度O(value)。其中n为数组nums的长度。
