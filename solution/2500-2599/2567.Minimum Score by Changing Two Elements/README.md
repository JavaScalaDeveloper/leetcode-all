...
因此，我们可以先对数组进行排序。由于题目允许我们修改数组中最多两个元素的值，我们可以通过修改一个数，让其跟数组中的另一个数相同，使得最小得分为0，那么数组 `nums` 的分数实际上就是最大得分。我们可以选择进行如下修改之一：

1. 修改最小的两个数为nums[2]，那么最大得分为nums[n - 1] - nums[2]；
1. 修改最小的一个数为nums[1]，最大的一个数为nums[n - 2]，那么最大得分为nums[n - 2] - nums[1]；
1. 修改最大的两个数为nums[n - 3]，那么最大得分为nums[n - 3] - nums[0]。
