...
我们用哈希表或数组cnt记录当前已经选择的数字以及它们的个数，用ans记录美丽子集的数目，初始时ans = -1，表示排除空集。

对于数组nums中的每个数字x，我们有两种选择：

-   不选择x，此时直接递归到下一个数字；
-   选择x，此时需要判断x + k和x - k是否已经在cnt中出现过，如果都没有出现过，那么我们就可以选择x，此时我们将x的个数加一，然后递归到下一个数字，最后将x的个数减一。

最后，我们返回ans即可。

时间复杂度O(2^n)，空间复杂度O(n)。其中n为数组nums的长度。
