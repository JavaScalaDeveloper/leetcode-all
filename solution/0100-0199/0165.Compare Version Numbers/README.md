...
同时遍历两个字符串，用两个指针i和j分别指向两个字符串的当前位置，初始时i = j = 0。

每次取出两个字符串中对应的修订号，记为a和b，比较a和b的大小，如果a \lt b，则返回-1；如果a \gt b，则返回1；如果a = b，则继续比较下一对修订号。

时间复杂度O(\max(m, n))，空间复杂度O(1)。其中m和n分别是两个字符串的长度。
