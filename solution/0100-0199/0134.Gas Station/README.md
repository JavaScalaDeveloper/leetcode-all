...
我们用i,j分别标记起点和终点，用s表示当前剩余汽油，而cnt表示当前行驶过的加油站数量。初始时，我们将起点设在最后一个位置，即i=n-1。

开始行驶，移动j。若发现当前剩余汽油小于0，说明当前i作为起点不符合要求，我们将起点i循环左移，并且更新剩余汽油，直至剩余汽油是非负数。

当行驶过的加油站数量达到n时，结束行驶过程。若此时剩余汽油s仍然小于0，说明不存在这样的起点，返回-1。否则，返回起点i。

时间复杂度O(n)，其中n表示加油站的数量。空间复杂度O(1)。
