...
如果k中两个字母相同，那么我们可以将\left \lfloor \frac{v}{2}  \right \rfloor \times 2个k连接到回文串的前后，此时如果k还剩余一个，那么我们可以先记录到x中。

如果k中两个字母不同，那么我们要找到一个单词k'，使得k'中的两个字母与k相反，即k' = k[1] + k[0]。如果k'存在，那么我们可以将\min(v, cnt[k'])个k连接到回文串的前后。
