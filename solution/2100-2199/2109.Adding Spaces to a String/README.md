...
我们可以用双指针i和j分别指向字符串s和数组spaces的头部，然后从头到尾遍历字符串s，当i等于spaces[j]时，我们往结果字符串中添加一个空格，然后j自增 1。接下来，我们将s[i]添加到结果字符串中，然后i自增 1。继续这个过程，直到遍历完字符串s。

时间复杂度O(n + m)，其中n和m分别是字符串s和数组spaces的长度。忽略答案的空间消耗，空间复杂度O(1)。
