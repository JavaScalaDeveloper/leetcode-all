...
我们创建一个数组ok，其中ok[i]表示nums[i]是否是可被二分搜索的。初始时ok[i]都为1。

我们先从左到右遍历数组，维护前缀最大值mx，如果当前元素x比mx小，那么x就不是可被二分搜索的，我们将ok[i]置为0，否则，我们将mx更新为x。

然后我们从右到左遍历数组，维护后缀最小值mi，如果当前元素x比mi大，那么x就不是可被二分搜索的，我们将ok[i]置为0，否则，我们将mi更新为x。

最后我们统计ok中的1的个数，即为可被二分搜索的元素的个数。
