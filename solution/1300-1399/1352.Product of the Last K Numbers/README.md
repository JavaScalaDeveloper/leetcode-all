...
我们初始化一个数组s，其中s[i]表示前i个数字的乘积。

当调用 `add(num)` 时，我们判断 `num` 是否为0，若是，则将s置为 `[1]`，否则将s的最后一个元素乘以 `num`，并将结果添加到s的末尾。

当调用 `getProduct(k)` 时，此时判断s的长度是否小于等于k，若是，则返回0，否则返回s的最后一个元素除以s的倒数第k + 1个元素。即s[-1] / s[-k - 1]。

时间复杂度O(1)，空间复杂度O(n)。其中n为调用 `add` 的次数。
