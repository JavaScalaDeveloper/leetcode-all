...
我们注意到数字范围不超过10^4，因此数位和的范围也不超过9 \times 4 = 36，因此我们可以用哈希表或者一个长度为40的数组cnt来统计每个数位和的个数，用一个变量mx表示最大的数位和个数。

我们在[1,..n]中枚举每个数，计算其数位和s，然后将cnt[s]加1，如果mx \lt cnt[s]，则更新mx = cnt[s]，并将ans置为1，如果mx = cnt[s]，则将ans加1。

最后返回ans即可。

时间复杂度O(n \times \log M)，空间复杂度(\log M)。其中n为给定的数字，而M是n的数字范围。
