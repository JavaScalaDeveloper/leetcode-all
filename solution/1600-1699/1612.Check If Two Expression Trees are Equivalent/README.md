...
我们定义一个计数器cnt，用于统计每个字母出现的次数。

然后我们分别对两棵二叉表达式树进行深度优先搜索，如果字母出现在左子树，则cnt中对应的字母的值加1，如果出现在右子树，则cnt中对应的字母的值减1。

最后，我们遍历cnt，如果所有字母的值都为0，则返回 `true`，否则返回 `false`。

时间复杂度O(n)，空间复杂度O(n)。其中n是二叉表达式树的节点个数。
