...
时间复杂度O(n\log n \times \log m)。其中n,m分别是数组 `nums` 的长度以及数组的最大元素。由于数组中的最大元素除以2的操作最多有O(\log m)次，因此全部元素除以2的操作最多有O(n\log m)次。每次弹出、放入堆的操作，时间复杂度为O(\log n)。因此，总的时间复杂度为O(n\log n \times \log m)。
