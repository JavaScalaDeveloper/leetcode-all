...
假设经过替换后，两数之和为x。由题意，我们知道x最小值为2，即两个数替换为1；最大值为2 × limit，即两个数都替换为limit。因此x的取值范围是[2,... 2 × limit]。
...
-   否则如果2 \le x \le 2 × limit，那么我们需要替换的次数为2，即把两个数都替换。

因此，我们可以遍历每一对数，执行如下操作：

1. 先将[2,... 2 × limit]范围需要的操作次数加2。
