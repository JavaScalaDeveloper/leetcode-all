...
我们定义f[i][j]表示将i个糖果分配给j个手袋的不同分配方式的数量。初始时f[0][0]=1，答案为f[n][k]。

我们考虑第i个糖果如何分配，如果第i个糖果分配给一个新的手袋，那么f[i][j]=f[i-1][j-1]；如果第i个糖果分配给一个已有的手袋，那么f[i][j]=f[i-1][j]\times j。因此，状态转移方程为：
...
时间复杂度O(n \times k)，空间复杂度O(n \times k)。其中n和k分别为糖果的数量和手袋的数量。
