...
因此，轮转操作最多产生n种状态，如果轮转位数b为偶数，累加操作只会对奇数位数字产生影响，因此总共产生n × 10种状态；如果轮转位数b为奇数，累加操作既会对奇数位数字产生影响，也会对偶数位数字产生影响，因此总共产生n × 10 × 10种状态。

所以，我们直接枚举所有的字符串状态，取字典序最小的状态即可。

时间复杂度O(n^2 × 10^2)，空间复杂度O(n)。其中n为字符串s的长度。
