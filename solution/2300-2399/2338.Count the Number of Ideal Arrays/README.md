...
我们注意到，maxValue的最大值不超过10^4，如果数组不含重复元素，那么数组最多只会有不超过16个元素。

**方法一：记忆化搜索 + 组合计数**

**方法二：动态规划**

设dp[i][j]表示以i结尾，且由j个不同元素构成的序列的方案数。初始值dp[i][1]=1。

考虑n个小球，最终划分为j份，那么可以用“隔板法”，即在n-1个位置上插入j-1个隔板，那么组合数为C_{n-1}^{j-1}。

我们可以预处理组合数C[i][j]，根据递推公式C[i][j]=C[i-1][j]+C[i-1][j-1]求得，特别地，当j=0时，C[i][j]=1。

最终的答案为\sum\limits_{i=1}^{k}\sum\limits_{j=1}^{\log_2 k + 1}dp[i][j] \times C_{n-1}^{j-1}。其中k表示数组的最大值，即maxValue。
