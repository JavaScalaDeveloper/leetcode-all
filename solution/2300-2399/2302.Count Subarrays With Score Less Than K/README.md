...
我们先计算出数组 `nums` 的前缀和数组s，其中s[i]表示数组 `nums` 前i个元素的和。

接下来，我们枚举数组 `nums` 每个元素作为子数组的最后一个元素，对于每个元素，我们可以通过二分查找的方式找到最大的长度l，使得s[i] - s[i - l] \times l < k。那么以该元素为最后一个元素的子数组个数即为l，我们将所有的l相加即为答案。

时间复杂度O(n \times \log n)，空间复杂度O(n)。其中n为数组 `nums` 的长度。

**方法二：双指针**

我们可以使用双指针的方式，维护一个滑动窗口，使得窗口内的元素和小于k。那么以当前元素为最后一个元素的子数组个数即为窗口的长度，我们将所有的窗口长度相加即为答案。
