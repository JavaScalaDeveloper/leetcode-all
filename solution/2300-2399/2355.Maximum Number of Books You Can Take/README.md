...
若从i到0不能取成一个公差为-1的等差数列，即这个规律在某个j处断掉了（0 \le j < i），那么一定有books[j] < books[i] - (i-j)，也即books[j] - j < books[i] - i，利用单调栈在新数组books[i] - i的每个位置，找到左边第一个比它严格小的数的位置，可以求出符合题意的j，此时dp[i]=dp[j] + \sum_{k=j+1}^{i} (books[k]-k)。
