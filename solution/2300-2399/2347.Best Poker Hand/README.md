...
我们可以先遍历数组suits，判断相邻两个元素是否均相等，如果是，则返回 `"Flush"`。

接下来，我们用哈希表或数组cnt统计每张牌的数量：

-   如果有任意一张牌的数量等于3，返回 `"Three of a Kind"`；
-   否则，如果有任意一张牌的数量等于2，返回 `"Pair"`；
-   否则，返回 `"High Card"`。

时间复杂度O(n)，空间复杂度O(n)。其中n为数组ranks的长度。
