...
枚举删除的位置i，那么每一个位置i的最长子数组长度为i左边连续的1的个数加上i右边连续的1的个数。

因此，我们可以先遍历一遍数组，统计每个位置i左边连续的1的个数，记录在 `left` 数组；然后再从右向左遍历一遍数组，统计每个位置i右边连续的1的个数，记录在 `right` 数组，最后枚举删除的位置i，求出最大值即可。
