...
遍历 `rains` 数组，对于每个下雨的日期i，如果 `rainy[rains[i]]` 存在，说明该湖泊在之前下过雨，那么我们需要找到 `sunny` 数组中第一个大于 `rainy[rains[i]]` 的日期，将其替换为下雨的日期，否则说明无法阻止洪水，返回空数组。对于没下雨的日期i，我们将i存入 `sunny` 数组中，并且将 `ans[i]` 置为 `1`。

遍历结束，返回答案数组。

时间复杂度O(n\log n)，空间复杂度O(n)。其中n为 `rains` 数组的长度。
