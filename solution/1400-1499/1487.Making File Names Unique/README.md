...
我们可以用哈希表d记录每个文件夹的最小可用编号，其中d[name] = k表示文件夹name的最小可用编号为k。初始时，d中没有任何文件夹，因此d为空。

接下来遍历文件夹数组，对于每个文件名name：

-   如果name在d中，说明文件夹name已经存在，我们需要找到一个新的文件夹名字。我们可以不断地尝试name(k)，其中k从d[name]开始，直到找到一个文件夹名字name(k)不存在于d中为止。我们将name(k)加入d中，并将d[name]更新为k + 1。然后我们将name更新为name(k)。
-   如果name不在d中，我们可以直接将name加入d中，并将d[name]更新为1。
-   接着我们将name加入答案数组。然后继续遍历下一个文件名。

遍历完所有文件名后，我们即可得到答案数组。

> 在以下代码实现中，我们直接修改文件名数组names，而不使用额外的答案数组。

时间复杂度O(L)，空间复杂度O(L)，其中L为数组names中所有文件名的长度之和。
