...
我们可以使用哈希表d记录前缀和最近一次出现的位置，初始时d[0]=0。

定义f[i]表示前i个元素中，长度和为target的最短子数组的长度。初始时f[0]=inf。

遍历数组 `arr`，对于当前位置i，计算前缀和s，如果s-target在哈希表中，记j=d[s-target]，则f[i]=min(f[i],i-j)，答案为ans=min(ans,f[j]+i-j)。继续遍历下个位置。
