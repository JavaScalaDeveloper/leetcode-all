...
然后我们枚举最小元素nums[i]，对于每个nums[i]，我们可以在nums[i + 1]到nums[n - 1]中找到最大元素nums[j]，使得nums[i] + nums[j] ≤ target，此时满足条件的子序列数目为2^{j - i}，其中2^{j - i}表示从nums[i + 1]到nums[j]的所有子序列的数目。我们将所有的子序列数目累加即可。
