...
题目求的是最高建筑物的高度，我们可以枚举相邻两个限制条件之间的建筑物i和i+1，要使得高度最大，那么高度应该是先增大后减小，假设最大高度为t，那么t - r_i[1] + t - r_{i+1}[1] ≤ r_{i+1}[0] - r_i[0]，即t ≤ \frac{r_i[1] + r_{i+1}[1] + r_{i+1}[0] - r_{i}[0]}{2}，我们取所有的t中的最大值即可。

时间复杂度O(m × log m)，空间复杂度O(m)。其中m为限制条件的数量。
