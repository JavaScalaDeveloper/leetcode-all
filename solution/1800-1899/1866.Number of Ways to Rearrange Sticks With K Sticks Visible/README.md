...
考虑最后一根木棍是否可以看到，如果可以看到，那么它一定是最长的，那么它的前面有i - 1根木棍，恰有j - 1根木棍可以看到，即f[i - 1][j - 1]；如果最后一根木棍不可以看到，那么它可以是除了最长的木棍之外的任意一根，那么它的前面有i - 1根木棍，恰有j根木棍可以看到，即f[i - 1][j] \times (i - 1)。
...
时间复杂度O(n \times k)，空间复杂度O(k)。其中n和k分别是题目中给定的两个整数。
