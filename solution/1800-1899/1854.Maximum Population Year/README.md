...
我们注意到，年份的范围是[1950,..2050]，因此我们可以将这些年份映射到一个长度为101的数组d中，数组的下标表示年份减去1950的值。

接下来遍历logs，对于每个人，我们将d[birth_i - 1950]加1，将d[death_i - 1950]减1。最后遍历数组d，求出前缀和的最大值，即为人口最多的年份，再加上1950即为答案。

时间复杂度O(n)，空间复杂度O(C)。其中n为数组logs的长度；而C为年份的范围大小，即2050 - 1950 + 1 = 101。
