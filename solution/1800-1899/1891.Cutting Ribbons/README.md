...
我们定义二分查找的左边界left=0,right=max(ribbons)，中间值mid=(left+right+1)/2，然后计算当前长度为mid的绳子能够得到的数量cnt，如果cnt ≥ k，说明我们能够得到长度为mid的k根绳子，那么我们将left更新为mid，否则我们将right更新为mid-1。

最后，我们返回left即可。

时间复杂度O(n × log M)，空间复杂度O(1)。其中n和M分别为绳子的数量和绳子的最大长度。
