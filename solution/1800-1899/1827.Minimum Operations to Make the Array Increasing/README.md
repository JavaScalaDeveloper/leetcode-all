...
我们用变量mx记录当前严格递增数组的最大值，初始时mx = 0。

从左到右遍历数组 `nums`，对于当前遍历到的元素v，如果v \lt mx + 1，那么我们需要将其增加到mx + 1，这样才能保证数组严格递增。因此，我们此次需要进行的操作次数为max(0, mx + 1 - v)，累加到答案中，然后更新mx=max(mx + 1, v)。继续遍历下一个元素，直到遍历完整个数组。
