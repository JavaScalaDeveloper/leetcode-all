...
我们可以用数组或哈希表cnt记录每个前缀和出现的次数，其中cnt[i]表示前缀和为i的子数组个数。初始时cnt[0] = 1。

接下来我们遍历数组 `nums`，用变量s维护当前的前缀和，对于每个s，我们可以计算出s - goal出现的次数，即为以当前位置结尾的满足条件的子数组个数，累加到答案中。然后我们将s的计数值加1。
