...
然后我们从小到大枚举横坐标，对于每个横坐标，我们枚举其对应的纵坐标中的所有点对(y_1, y_2)，其中y_1 \lt y_2。如果我们之前已经遇到过点对(y_1, y_2)，那么就可以用当前的横坐标和之前的横坐标计算出一个矩形的面积。我们用哈希表pos记录每个点对(y_1, y_2)第一次出现的横坐标，这样我们就可以在常数时间内找到这个横坐标。

最后，我们返回所有矩形的面积中的最小值。

时间复杂度O(n^2 \times \log n)，空间复杂度O(n^2)。其中n是点的数量。
