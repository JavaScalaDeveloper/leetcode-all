...
我们用x表示字符串中待匹配的左括号的数量，初始时为0。遍历字符串s：

如果遇到左括号，则x的值加1；如果遇到右括号，我们分情况讨论：

-   如果有两个连续的右括号，那么我们先让指针往后移动一位；否则，我们需要插入一个右括号，使得出现两个连续的右括号，因此插入次数加1；
-   如果x = 0，说明当前没有待匹配的左括号，我们需要插入一个左括号，用于匹配上面准备好的两个连续的右括号，因此插入次数加1；否则，我们让x的值减1。

然后指针往后移动一位，继续下一次遍历。

遍历结束后，如果x = 0，说明字符串已经平衡，返回插入次数；否则，说明字符串中有待匹配的左括号，我们需要再插入2 \times x个右括号，使得字符串变成平衡字符串，返回插入次数。
