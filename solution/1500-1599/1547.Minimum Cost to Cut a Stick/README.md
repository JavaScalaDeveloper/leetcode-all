...
否则，我们枚举区间的长度l，其中l等于切割点的数量减去1。然后我们枚举区间的左端点i，右端点j可以由i + l得到。对于每个区间，我们枚举它的切割点k，其中i < k < j，那么我们可以将区间[i, j]切割为[i, k]和[k, j]，此时的成本为f[i][k] + f[k][j] + cuts[j] - cuts[i]，我们取所有可能的k中的最小值，即为f[i][j]的值。
