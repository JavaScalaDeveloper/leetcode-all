...
我们用哈希表d存储每个单词在数组中出现的所有下标，然后用双指针i和j分别指向两个单词在数组中出现的下标列表a和b，每次更新下标差值的最小值，然后移动下标较小的指针，直到其中一个指针遍历完下标列表。

初始化的时间复杂度为O(n)，其中n为数组的长度。每次调用 `shortest` 方法的时间复杂度为O(m + n)，其中m为两个单词在数组中出现的下标列表的长度之和。
