...
题目中需要判断链表中节点的值是否在数组 `nums` 中，因此我们可以使用哈希表s存储数组 `nums` 中的值。

然后遍历链表，找到第一个在哈希表s中的节点，然后从该节点开始遍历，直到遇到不在哈希表s中的节点，这样就找到了一个组件，然后继续遍历链表，直到遍历完整个链表。

时间复杂度O(n)，空间复杂度O(n)。其中n为链表的节点个数。
