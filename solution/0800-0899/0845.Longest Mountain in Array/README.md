...
我们定义两个数组f和g，其中f[i]表示以arr[i]结尾的最长上升子序列的长度，而g[i]表示以arr[i]开头的最长下降子序列的长度。那么对于每个下标i，如果f[i] \gt 1且g[i] \gt 1，那么以arr[i]为山顶的山脉的长度为f[i] + g[i] - 1，我们只需要枚举所有的i，找出最大的那个值即可。

时间复杂度O(n)，空间复杂度O(n)。其中n为数组arr的长度。

**方法二：一次遍历（枚举左侧山脚）**

我们可以枚举山脉的左侧山脚，然后向右寻找山脉的右侧山脚。我们可以使用两个指针l和r，其中l表示左侧山脚的下标，r表示右侧山脚的下标，初始时l=0,r=0，然后我们向右移动r，找到山顶的位置，此时判断r是否满足r + 1 \lt n并且arr[r] \gt arr[r + 1]，如果满足，我们向右继续移动r，直到找到右侧山脚的位置，此时山脉的长度为r - l + 1，我们更新答案，然后将l的值更新为r，继续寻找下一个山脉。
