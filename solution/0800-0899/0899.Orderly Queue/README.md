...
若k\gt1，对于形如abc[xy]def的字符串，可以依次将a,b,c移动到最后，得到[xy]defabc，然后将y,x移动到最后，得到defabc[yx]，最后将d,e,f移动到最后，得到abc[yx]def，这样就实现了对y,x的交换。

因此，只要k\gt1，我们就能够交换字符串中的任何两个相邻字符，最终得到一个升序排列的字符串。
