...
我们可以枚举 `arr` 中的每一个数a作为二叉树的根节点（根节点一定最大），然后枚举枚举左子树的值b，若a能被b整除，则右子树的值为a / b，若a / b也在 `arr` 中，则可以构成一棵二叉树。此时，以a为根节点的二叉树的个数为f(a) = f(b) × f(a / b)，其中f(b)和f(a / b)分别为左子树和右子树的二叉树个数。
