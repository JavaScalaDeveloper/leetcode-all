...
考虑一个x\times x的正方形，我们需要在正方形里面取最多 `maxOnes` 个点，将其置为 1。注意到当坐标(i, j)处的点被选取后，所有坐标为(i\pm k_1 \times x, j\pm k_2 \times x)的点都可以等效地置为 1。因此，我们算出坐标(i, j)在矩阵中的等效位置的数量，取数量最多的前 `maxOnes` 个即可。

时间复杂度O(m\times n)。其中m,n分别是矩阵的行数和列数。
