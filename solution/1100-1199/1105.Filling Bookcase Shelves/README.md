...
-   如果这本书可以与前面的最后几本书摆放在同一层，我们从后往前枚举同一层的第一本书boos[j-1]，其中j \in [1, i - 1]，将书的厚度累积到w，如果w \gt shelfWidth，说明此时的books[j-1]已经无法与books[i-1]摆放在同一层，停止枚举；否则我们更新当前层的最大高度h = \max(h, books[j-1][1])，那么此时有f[i] = \min(f[i], f[j - 1] + h)。
