...
若链表节点的两个前缀和相等，说明两个前缀和之间的连续节点序列的和为0，那么可以消去这部分连续节点。

第一次遍历链表，用哈希表 `last` 记录前缀和以及对应的链表节点，同一前缀和s，**后者的链表节点覆盖前者**。

第二次遍历链表，若当前节点 `cur` 的前缀和s在 `last` 出现，说明 `cur` 与 `last[s]` 之间的所有节点和为0，直接修改 `cur` 的指向，`cur.next = last[s].next`，就删去了这部分和为0的连续节点。继续往后遍历，删除所有和为0的连续节点。
