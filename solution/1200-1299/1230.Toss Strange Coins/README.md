...
考虑f[i][j]，其中i \geq 1，如果当前硬币反面朝上，那么f[i][j] = (1 - p) \times f[i - 1][j]；如果当前硬币正面朝上，并且j \gt 0，那么f[i][j] = p \times f[i - 1][j - 1]。因此状态转移方程为：
...
时间复杂度O(n \times target)，空间复杂度O(target)。其中n为硬币的数量。
