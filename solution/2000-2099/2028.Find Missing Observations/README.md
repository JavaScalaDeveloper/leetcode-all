...
根据题目描述，所有数字之和为(n + m) × mean，已知的数字之和为 `sum(rolls)`，那么缺失的数字之和为s = (n + m) × mean - sum(rolls)。

如果s > n × 6或者s < n，说明不存在满足条件的答案，返回空数组。

否则，我们可以将s平均分配到n个数字上，即每个数字的值为s / n，其中s mod n个数字的值再加上1。
