...
bank.withdraw(3, 10);    // 返回 true ，账户 3 的余额是20 ，所以可以取款10 。
                         // 账户 3 余额为20 -10 =10 。
bank.transfer(5, 1, 20); // 返回 true ，账户 5 的余额是30 ，所以可以转账20 。
                         // 账户 5 的余额为30 -20 =10 ，账户 1 的余额为10 +20 =30 。
bank.deposit(5, 20);     // 返回 true ，可以向账户 5 存款20 。
                         // 账户 5 的余额为10 +20 =30 。
bank.transfer(3, 4, 15); // 返回 false ，账户 3 的当前余额是10 。
                         // 所以无法转账15 。
...
以上操作的时间复杂度均为O(1)，空间复杂度为O(n)。其中，n为 `balance` 的长度。
