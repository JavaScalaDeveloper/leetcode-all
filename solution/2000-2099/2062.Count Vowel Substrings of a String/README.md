...
我们可以枚举子字符串的左端点i，对于当前左端点，维护一个哈希表，记录当前子字符串中出现的元音字母，然后枚举右端点j，如果当前右端点对应的字母不是元音字母，则跳出循环，否则将当前右端点对应的字母加入哈希表，如果哈希表中的元素个数为5，则说明当前子字符串是一个元音子字符串，将结果加1。

时间复杂度O(n^2)，空间复杂度O(C)。其中n为字符串word的长度；而C为字符集大小，本题中C=5。
