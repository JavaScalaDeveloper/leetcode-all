...
然后我们枚举在[0,..n-1]范围内枚举所有i，若i满足i - k ≥ 0并且i + k < n，我们将ans[i]更新为\frac{s[i + k + 1] - s[i - k]}{k × 2 + 1}。
...
我们维护一个大小为k × 2 + 1的窗口，记窗口中的所有元素和为s。

与方法一一样，我们创建一个长度为n的答案数组ans，初始时每项元素均为-1。

接下来遍历数组 `nums`，将nums[i]的值加到窗口的和s中，如果此时i ≥ k × 2，说明此时窗口大小为k × 2 + 1，那么ans[i-k] = \frac{s}{k × 2 + 1}，然后我们将nums[i - k × 2]的值从窗口和s中移出。继续遍历下个元素。
