...
我们可以递归判断当前节点root与key的大小关系：

1. 若root.val>key，则递归左子树；
1. 若root.val<key，则递归右子树；
1. 若root.val=key，则进一步判断：
    1. 若root没有左子树，则root.right顶替root的位置；
    1. 若root没有右子树，则root.left顶替root的位置；
    1. 若root同时存在左右子树，则将左子树转移至右子树的最左节点的左子树上，然后root.right顶替root的位置。
