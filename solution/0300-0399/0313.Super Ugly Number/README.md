...
我们用一个优先队列（小根堆）维护所有可能的超级丑数，初始时将1放入队列中。

每次从队列中取出最小的超级丑数x，将x乘以数组 `primes` 中的每个数，将乘积放入队列中，然后重复上述操作n次即可得到第n个超级丑数。

由于题目保证第n个超级丑数在32位带符号整数范围内，因此，我们将乘积放入队列之前，可以先判断乘积是否超过2^{31} - 1，如果超过，则不需要将乘积放入队列中。另外，可以使用欧拉筛优化。

时间复杂度O(n \times m \times \log (n \times m))，空间复杂度O(n \times m)。其中m和n分别为数组 `primes` 的长度和给定的整数n。
