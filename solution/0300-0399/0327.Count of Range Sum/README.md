...
题目要求区间和，因此我们可以先求出前缀和数组s，其中s[i]表示nums中前i个元素的和。那么对于任意的i \lt j，s[j+1] - s[i]就是nums中下标在[i, j]的元素之和。

而lower \leq s[j+1] - s[i] \leq upper，可以转换为s[j+1] - upper \leq s[i] \leq s[j+1] - lower，也就是说，对于当前前缀和s[j+1]，我们需要统计s中有多少个下标i满足s[j+1] - upper \leq s[i] \leq s[j+1] - lower。

我们可以用树状数组来维护每个前缀和出现的次数，这样对于每个前缀和s[j+1]，我们只需要查询树状数组中有多少个前缀和s[i]满足s[j+1] - upper \leq s[i] \leq s[j+1] - lower即可。

时间复杂度O(n \times \log n)，空间复杂度O(n)。其中n为数组长度。
